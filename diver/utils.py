...
class UniverseUtils:
        ...
        if self.wait_fig(lambda: not self.check("use_package", 0.5182, 0.9407), 3):
            ...
            if self.wait_fig(
                ...
                if self.wait_fig(
                    ...
                    self.get_screen()
                    if self.wait_fig(
                        lambda: not self.check(
                            "use_star", 0.8828, 0.8648, threshold=0.9
                        ),
                        0.6,
                    ):
                        self.use_it(x, y)
        ...
        # 寻路模式
            ...
            # 离目标点挺近了,准备找下一个目标点
                ...
                try:
                    ...
                    # 目标点可能已被其他线程移除,忽略
                    log.debug(f"目标点已不在列表中: {(loc, type)}")
    ...
    def match_scr(self, img):
        ...
        for i, j in self.img_set:
            try:
                ...
                # 特征匹配失败(可能是描述符为空),跳过该地图
                log.debug(f"地图 {i} 特征匹配失败: {e}")
    ...
    def blessing(self):
        self.get_screen()
        if self.wait_fig(
            lambda: not self.check("choose_blessing", 0.9266, 0.9491), 2.3
        ):
        ...
        for _ in range(6):
            ...
            if self.check("reset", 0.2938, 0.0954, threshold=0.96):
                for _ in range(14):
                    ...
                    if (
                        ...
                        break
                    if not self.check(
                        "choose_blessing", 0.9266, 0.9491, threshold=0.945
                    ):
                        return 1
                    time.sleep(0.2)
                self.get_screen()
            ...
            time.sleep(1.7)
            self.get_screen()
            if not self.check("choose_blessing", 0.9266, 0.9491, threshold=0.945):
                return

    def get_text_position(self, clean=0):
        if self.event_mask is None:
            self.event_mask = (
                cv.imread("imgs/divergent/event_mask.jpg", cv.IMREAD_GRAYSCALE) > 70
            )[:497]
            self.event_mask_clean = (
                cv.imread("imgs/divergent/event_mask_clean.jpg", cv.IMREAD_GRAYSCALE)
                > 70
            )[:497]
        scr = self.screen[:497]
