...
def is_edit_distance_at_most_one(str1: str, str2: str, ch: str) -> int:
    ...
    Example:
        >>> is_edit_distance_at_most_one("黄泉", "黄泉", "")
        1
        >>> is_edit_distance_at_most_one("黄泉", "黄权", "")
        1
        >>> is_edit_distance_at_most_one("黄泉", "白泉", "")
        1
        >>> is_edit_distance_at_most_one("黄泉", "白露", "")
        0
    """
    length = len(str1)
...
def fuzzy_match(target: str, text: str) -> bool:
    ...
    for i in range(len(search_text) - length):
        if is_edit_distance_at_most_one(target, search_text[i:i + length], search_text[i + length]):
            return True
