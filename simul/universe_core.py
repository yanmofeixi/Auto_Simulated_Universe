...
class SimulatedUniverse(UniverseUtils):
        ...
        if self.check("choose_blessing", 0.9266, 0.9491):
            ...
            if self.check("reset", 0.2938, 0.0954):
                for _ in range(14):
                    ...
                    if (
                        self.ts.split_and_find(
                            self.tk.fates, img_down, mode="blessing"
                        )[1]
                        or self._stop
                    ):
                        time.sleep(0.2)
                        break
                    if not self.check("choose_blessing", 0.9266, 0.9491):
                        return 1
                    time.sleep(0.2)
                self.get_screen()
                img_up = self.check(
                    "z", 0.5047, 0.5491, mask="mask_blessing", large=False
                )
                res_up = self.ts.split_and_find(
                    self.tk.prior_blessing, img_up, blessing_skip=self.tk.skip
                )
                img_down = self.check("z", 0.5042, 0.3204, mask="mask", large=False)
                res_down = self.ts.split_and_find(
                    [self.fate], img_down, mode="blessing"
                )
                if res_up[1] == 2:
            ...
            if not chose:
                ...
                img_down = self.check("z", 0.5042, 0.3204, mask="mask", large=False)
                res_down = self.ts.split_and_find(
                    self.tk.secondary, img_down, mode="blessing"
                )
                if res_up[1] == 2:
                    self.click(self.calc_point((0.5047, 0.5491), res_up[0]))
                elif res_down[1] >= 2:
            ...
            tm = time.time()
            while time.time() - tm < 1.6 and self.check(
                "choose_blessing", 0.9266, 0.9491
            ):
                time.sleep(0.1)
                self.get_screen()
    ...
    def get_target(self, pth):
        ...
        res = set()
        f_set = [
            lambda p: p[2] < 85 and p[1] < 85 and p[0] > 180,  # 路径点:蓝色
            lambda p: p[2] > 180 and p[1] < 70 and p[0] < 70,  # 怪物:红色
            lambda p: p[2] < 90 and p[1] > 150 and p[0] < 90,  # 交互点:绿色
            lambda p: p[2] > 180 and p[1] > 180 and p[0] < 70,  # 终点:黄色
        ]
